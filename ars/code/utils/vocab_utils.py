...
def _setup_args():
    ...
    embed_file = os.path.join('download', 'dwr', 'glove.6B.50d.txt')
    parser.add_argument("--corpus_dir", default=corpus_dir)
    parser.add_argument("--vocab_dir", default=vocab_dir)
    parser.add_argument("--embed_file", default=embed_file)
...
def create_vocab_file(vocabulary_path, embed_file, corpus_paths, tokenizer=None):
    if not tf.gfile.Exists(vocabulary_path):
        print("Creating vocabulary %s from data %s" % (vocabulary_path, str(corpus_paths)))
        vocab = collections.defaultdict(int)
        embed_dict, _ = load_embed_txt(embed_file)
            ...
            with codecs.getreader("utf-8")(tf.gfile.GFile(path, mode="rb")) as f:
                ...
                for line in f:
                    ...
                    for w in tokens:
                        ...
                        w_up = w.upper()
                        w_lo = w.lower()
                        if w in embed_dict: vocab[w] += 1
                        if w_cap in embed_dict: vocab[w_cap] += 1
                        if w_up in embed_dict: vocab[w_up] += 1
                        if w_lo in embed_dict: vocab[w_lo] += 1
...
if __name__ == '__main__':
    ...
    create_vocab_file(vocab_path, embed_file,
